...
class BERT(nn.Module):
    def __init__(self, name: str, train_bert: bool, hidden_dim: int, max_len: int, enc_num, med_config = 'configs/med_config.json',config = None,mha_adapters = None,mlp_adapters=None):
        super().__init__()
    ...
    def forward(self,text_data_tensors, text_data_mask):
        ...
        return out,mha_mlp_adapter,embedding_output

def build_bert(args,config,mha_adapters = None,mlp_adapters = None):
    # position_embedding = build_position_encoding(args)
    train_bert = args.lr_bert > 0

    # lt
    
    bert = BERT(args.bert_model, train_bert, args.hidden_dim, args.max_query_len, args.bert_enc_num,config = config,mha_adapters = mha_adapters)
