...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--data_root', type=str, default='/share/home/liuting/transvg_data/ln_data',
                        help='path to ReferIt splits data folder')
    parser.add_argument('--split_root', type=str, default='/share/home/liuting/transvg_data/data',
    ...
    parser.add_argument('--detr_model', default='./checkpoints/detr-r50-referit.pth', type=str, help='detr model')
...
def main(args,config):
    ...
    viusal_model = model.visumodel
    for name,parameter in viusal_model.named_parameters():
        if 'adapter' in name:
            parameter.requires_grad = True
        elif 'vis_downsample' in name:
            parameter.requires_grad = True
    ...
    vis_ada_parameters = sum(p.numel() for p in model.visumodel.parameters() if p.requires_grad)
    text_ada_parameters = sum(p.numel() for p in model.textmodel.parameters() if p.requires_grad)
    print('vis_ada_parameters:', vis_ada_parameters)
    print('text_ada_parameters:', text_ada_parameters)
    print('all_ada_parameters:', text_ada_parameters+vis_ada_parameters)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                     **{f'validation_{k}': v.tolist() for k, v in val_stats.items()},
                     'epoch': epoch,
