...
def convert_examples_to_features(examples, seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        input_type_ids = []
        tokens.append("[CLS]")
        # input_type_ids表示每个输入token对应的句子类型。同一个句子的ids相同
        ...
        if len(input_ids) < seq_length:
            ...
            input_mask += [0] * (seq_length - len(input_mask))
            input_type_ids += [0] * (seq_length - len(input_type_ids))
